...
impl Movie {
    ...
    fn system(
        ...
        mut movies: Query<(Option<&mut AnimatedImageController>, &mut Movie, Entity)>,
        ...
        for (controller_option, mut movie, entity) in &mut movies {
            ...
            if controller_option.is_none() && movie.audio_ins.is_none() {
                commands
                    .entity(entity)
                    .insert(AnimatedImageController::play(movie.video_src.clone()));
                movie.audio_ins = Some(audio.play(movie.audio_src.clone()).handle());
            } else if let (Some(audio_instance), Some(mut controller)) = (
                audio_assets.get_mut(movie.audio_ins.as_ref().expect("Must be set")),
                controller_option,
            ) {
                // Check if both are stopped
                let video_stopped = controller.current_frame() == controller.frame_count()
                    || controller.play_count() >= 1;
                let audio_stopped = audio_instance.state() == PlaybackState::Stopped;

                // Stop movie from looping if it ended
                if video_stopped && !controller.paused() {
                    controller.pause();
                }
                ...
                if video_stopped && audio_stopped && movie.state != MovieState::Ended {
                    ...
                    match movie.playback {
                        ...
                        _ => movie.state = MovieState::Ended,
                    }
                }
            }
        }
...
#[allow(dead_code)] // TODO: Implement repeat
